...
def meta_stacking_with_heikin_ashi():
    ...
    # Phân tích xu hướng với Heikin Ashi cho các khung thời gian M5, M15, H1
    X_m5, y_m5 = analyze_trend_with_heikin(Client.KLINE_INTERVAL_5MINUTE, client)
    X_m15, y_m15 = analyze_trend_with_heikin(Client.KLINE_INTERVAL_15MINUTE, client)
    X_h1, y_h1 = analyze_trend_with_heikin(Client.KLINE_INTERVAL_1HOUR, client)

    # Đồng bộ kích thước của các tập dữ liệu
    min_len = min(len(X_m5), len(X_m15), len(X_h1))
    X_combined = np.hstack([X_m5[-min_len:], X_m15[-min_len:], X_h1[-min_len:]])
    y_combined = y_m5[-min_len:]  # Giả sử nhãn giống nhau
    ...
    if prediction_prob[1] > 1 - threshold:
        ...
        print(f"TakeProfit: {btc_price['price']} USDT")
        playsound(r"C:\Users\DELL\Desktop\GPT train\TP.mp3")
